...
class LogLevelAdjuster:
    ...
    def __init__(self, max_workers: int = 1, error_logs_only: bool = False):
        """Initialize the LogLevelAdjuster.
        
        Args:
            max_workers: Maximum number of worker threads for parallel processing
            error_logs_only: If True, only adjust error logs outside catch blocks
        """
        self.max_workers = max_workers
        self.error_logs_only = error_logs_only
        self.total_files = 0
    ...
    def determine_appropriate_level(self, current_level: str, message: str, context_info: Dict, 
        ...
        # Special handling for error logs in catch blocks
        if in_catch_block:
            # If we're in a catch block, check if the message indicates an error
            ...
            # If it's already an error log in a catch block, keep it as error
            if current_level == "error":
                return "error"
        
        # If we're only adjusting error logs and this is not an error log outside a catch block
        if self.error_logs_only:
            if current_level != "error" or in_catch_block:
        ...
        # 3. Check for error patterns (ERROR)
        for indicator in ERROR_INDICATORS:
            if indicator in message_lower:
                return "error"
        
        # 4. Check for fatal patterns (FATAL)
        for indicator in FATAL_INDICATORS:
            if indicator in message_lower:
                return "fatal"
        
        # 5. Check for warning patterns (WARN)
        for indicator in WARN_INDICATORS:
            if indicator in message_lower:
                return "warn"
        
        # 6. Check for info patterns (INFO)
        for indicator in INFO_INDICATORS:
            if indicator in message_lower:
                return "info"
        
        # 7. Analyze conditional logic
        if in_conditional:
        ...
        # 8. Analyze related logs for patterns
        if related_logs:
        ...
        # 10. If message contains data structure dumps or technical details
...
def main():
    ...
    # Ask if user wants to only adjust error logs
    error_logs_only = input("Do you want to only adjust error logs (fixing misplaced error logs and ensuring catch blocks have error logs)? (y/n): ").strip().lower() == 'y'
    ...
    adjuster = LogLevelAdjuster(max_workers=workers, error_logs_only=error_logs_only)
